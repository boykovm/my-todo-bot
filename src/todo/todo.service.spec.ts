...
describe('TodoService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [TodoService, { provide: PrismaService, useValue: mockPrismaService }],
    }).compile();
  ...
  describe('create', () => {
    ...
    it('should pass DTO fields through to prisma without modification', async () => {
      mockPrismaService.todo.create.mockResolvedValue(mockTodo);
  ...
  describe('findAll', () => {
    it('should call prisma.todo.findMany with deleted: false and return the list', async () => {
      const todos: Todo[] = [mockTodo, { ...mockTodo, id: 2, text: 'Another todo' }];
  ...
  describe('findOne', () => {
    it('should call prisma.todo.findUnique with correct id and return the todo', async () => {
      ...
      expect(mockPrismaService.todo.findUnique).toHaveBeenCalledWith({ where: { id: 1 } });
      expect(result).toStrictEqual(mockTodo);
    });
  ...
  describe('update', () => {
    it('should call prisma.todo.update with correct id and data, then return updated todo', async () => {
      const updatedTodo: Todo = { ...mockTodo, text: 'Updated text', isDone: true };
      mockPrismaService.todo.findUnique.mockResolvedValue(mockTodo);
  ...
  describe('findByOwner', () => {
    it('should return todos filtered by ownerId and not deleted', async () => {
      const ownerTodos: Todo[] = [mockTodo];
      ...
      expect(result).toStrictEqual(ownerTodos);
    });
  });
  ...
  describe('findDueToday', () => {
    it('should query todos with deadline within today, not done, not deleted', async () => {
      ...
      // Verify structure without exact timestamps (to avoid midnight race conditions)
      expect(mockPrismaService.todo.findMany).toHaveBeenCalledWith({
        where: {
          deleted: false,
          isDone: false,
      ...
      // Verify the time bounds are correct for the current day
      ...
      type FindManyCall = [{ where: FindManyWhere }];
      const [[{ where }]] = mockPrismaService.todo.findMany.mock.calls as FindManyCall[];

      const gte = new Date(where.deadline.gte);
      const lte = new Date(where.deadline.lte);
